...
public abstract class Module {
    ...
    private int cpsClicks;
    private long cpsWindow;
    ...
    /** Full toggle — fires lifecycle, shows toast, saves config. */
    public void setEnabled(boolean enabled) {
        ...
        try {
            ...
            PixelForgeClient.LOGGER.error("Module {} lifecycle failed", name, t);
        }
        if (PixelForgeClient.getInstance() != null && PixelForgeClient.getInstance().getNotificationManager() != null)
            PixelForgeClient.getInstance().getNotificationManager().push(
                name + (enabled ? " enabled" : " disabled"), enabled ? 0x55FF55 : 0xFF5555);
        try { ConfigManager.saveModule(this); } catch (Throwable ignored) {}
    }

    /**
     * Sets enabled state during config load.
     * Fires lifecycle callbacks so modules restore their state correctly,
     * but skips the toast notification and does NOT re-save (avoids pointless I/O at startup).
     */
    public void setEnabledSilent(boolean enabled) {
        ...
        try {
            ...
            PixelForgeClient.LOGGER.error("Module {} lifecycle failed during load", name, t);
        }
    }
    ...
    public void onEnable() {
        if (mc == null) return;
        if (id().equals("fovchanger")) {
            previousFov = mc.options.getFov().getValue();
            mc.options.getFov().setValue(110);
        }
        if (id().equals("cpstrainer")) {
            cpsClicks = 0;
            cpsWindow = System.currentTimeMillis();
        }
    }

    public void onDisable() {
        if (mc == null) return;
        if (id().equals("fovchanger") && previousFov != null) {
            mc.options.getFov().setValue(previousFov);
            previousFov = null;
        }
        if (id().equals("togglesprint") && mc.player != null) mc.player.setSprinting(false);
        if (id().equals("togglesneak")  && mc.player != null) mc.player.setSneaking(false);
    }
    ...
    private void setItemUseCooldown(int value) {
        try {
            var f = MinecraftClient.class.getDeclaredField("itemUseCooldown");
            f.setAccessible(true);
            f.setInt(mc, value);
        } catch (Throwable ignored) {}
    }

    private void setMinimalParticles() {
        try {
            var option = mc.options.getParticles();
            if (option.getValue() != ParticlesMode.MINIMAL) option.setValue(ParticlesMode.MINIMAL);
        } catch (Throwable ignored) {}
    }
    ...
    private void tickCpsTrainer() {
        ...
        if (left && !previousLeft) cpsClicks++;
        previousLeft = left;
        if (cpsWindow == 0) cpsWindow = now;
        if (now - cpsWindow >= 1000) { cpsWindow = now; cpsClicks = 0; }
    }

    private void tickBlockHitTrainer() {
        boolean use = mc.options.useKey.isPressed();
        if (use && !previousUse && mc.player.isUsingItem()) mc.player.swingHand(Hand.MAIN_HAND);
        previousUse = use;
    }

    private void tickWTapTrainer() {
        if (mc.options.forwardKey.isPressed() && mc.player.isSprinting() && mc.player.horizontalCollision)
            mc.player.setSprinting(false);
    }

    private void tickStrafeTrainer() {
        boolean strafe = mc.options.leftKey.isPressed() || mc.options.rightKey.isPressed();
        if (mc.player.isOnGround() && mc.options.forwardKey.isPressed() && strafe)
            if (!mc.player.isSprinting()) mc.player.setSprinting(true);
    }

    private void tickAimTrainer() {
        boolean attack = mc.options.attackKey.isPressed();
        if (mc.crosshairTarget != null && attack && !previousAttack) mc.player.swingHand(Hand.MAIN_HAND);
