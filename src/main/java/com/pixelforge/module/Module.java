...
public abstract class Module {
    ...
    private Double previousGamma; private Integer previousFov; private int cleanerTicks; private boolean previousLeft; private int cpsClicks; private long cpsWindow;
    private boolean previousUse; private boolean previousAttack;
    ...
    public void onEnable(){
        ...
        if(name.equals("CpsTrainer")){cpsClicks=0;cpsWindow=System.currentTimeMillis();}
    }
    ...
    private void autoEat(){
        ...
        int old=mc.player.getInventory().getSelectedSlot(),best=-1;
    ...
    private void autoTool(){
        if(mc.world==null||!(mc.crosshairTarget instanceof BlockHitResult hit))return;
    ...
    @SuppressWarnings({"rawtypes","unchecked"}) private void setMinimalParticles(){try{var option=mc.options.getParticles();Object current=option.getValue();if(current instanceof Enum e&&e.name().equals("ALL"))option.setValue(Enum.valueOf((Class)current.getDeclaringClass(),"MINIMAL"));}catch(Throwable ignored){}}
    private void tickCpsTrainer(){
        long now=System.currentTimeMillis();boolean left=GLFW.glfwGetMouseButton(mc.getWindow().getHandle(),GLFW.GLFW_MOUSE_BUTTON_LEFT)==GLFW.GLFW_PRESS;
        if(left&&!previousLeft)cpsClicks++;previousLeft=left;if(cpsWindow==0)cpsWindow=now;if(now-cpsWindow>=1000){
            cpsWindow=now;cpsClicks=0;
        }
    }
    ...
    private void tickStrafeTrainer(){if(mc.player.isOnGround()&&mc.player.forwardSpeed>0&&mc.player.sidewaysSpeed!=0)mc.player.setSprinting(mc.player.isSprinting());}
    private void tickAimTrainer(){if(mc.crosshairTarget!=null&&mc.options.attackKey.isPressed()&&!previousAttack)mc.player.swingHand(Hand.MAIN_HAND);previousAttack=mc.options.attackKey.isPressed();}
    public void onRender(DrawContext context,float tickDelta){
        if(category!=Category.TRAINER||!enabled)return;
        int y=8;String text;
        if(name.equals("CpsTrainer")){text="CPS Trainer · clicks/s: "+cpsClicks;}
        else if(name.equals("AimTrainer")){text="Aim Trainer · keep your crosshair on target";}
        else if(name.equals("WTapTrainer")){text="W-Tap Trainer · sprint timing active";}
        else if(name.equals("BlockHitTrainer")){text="Block-Hit Trainer · practice block timing";}
        else if(name.equals("StrafeTrainer")){text="Strafe Trainer · A/D + forward";}
        else if(name.equals("KnockbackTrainer")){text="Knockback Trainer · movement feedback active";}
        else if(name.equals("PingSimulator")){text="Ping Simulator · training mode";}
        else{text=name+" · training mode";}
        RenderUtil.fill(context,8,y,8+Math.min(320,context.getScaledWindowWidth()-16),y+16,0xB0101424);RenderUtil.drawBorder(context,8,y,Math.min(320,context.getScaledWindowWidth()-16),16,0xFF3B5BDB);RenderUtil.drawText(context,mc.textRenderer,text,14,y+4,0xFFC8D0E0,false);
    }
