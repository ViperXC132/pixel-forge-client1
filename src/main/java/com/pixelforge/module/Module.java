...
public abstract class Module {
    ...
    private void autoEat(){if(mc.interactionManager==null||mc.player.isUsingItem()||mc.player.getHungerManager().getFoodLevel()>=14)return;int old=mc.player.getInventory().getSelectedSlot(),best=-1;for(int i=0;i<9;i++){ItemStack s=mc.player.getInventory().getStack(i);if(!s.isEmpty()&&s.getItem().isFood()){best=i;break;}}if(best>=0){mc.player.getInventory().setSelectedSlot(best);mc.interactionManager.interactItem(mc.player,Hand.MAIN_HAND);mc.player.getInventory().setSelectedSlot(old);}}
