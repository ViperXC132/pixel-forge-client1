...
/** Persistent module state and typed settings storage. */
public class ConfigManager {
        ...
        try {
            ...
            if (obj.has("settings") && obj.get("settings").isJsonObject()) {
                ...
                for (Module.Setting<?> setting : module.getSettings()) {
                    if (settings.has(setting.getName())) applySetting(setting, settings.get(setting.getName()));
                }
            }

            // Apply enabled state last so onEnable() sees fully restored settings.
            if (obj.has("enabled")) {
    ...
    private static void applySetting(Module.Setting setting, com.google.gson.JsonElement value) {
        try {
            ...
            // A bad individual setting must not prevent the rest of the client loading.
        }
    }
    ...
    public void saveAll() {
        for (Module module : PixelForgeClient.getInstance().getModuleManager().getModules()) saveModule(module);
    }
