...
public class ModsScreen extends Screen {
    ...
    private List<ModContainer> installed(){String q=searchField==null?"":searchField.getText().trim().toLowerCase(Locale.ROOT);return FabricLoader.getInstance().getAllMods().stream().filter(m->{String id=m.getMetadata().getId().toLowerCase(Locale.ROOT);if(LIBRARIES.contains(id))return false;String n=m.getMetadata().getName().toLowerCase(Locale.ROOT);return q.isEmpty()||id.contains(q)||n.contains(q);}).toList();}
    ...
    private void drawIcon(DrawContext c,String modId,int x,int y,int size){try{ModContainer container=FabricLoader.getInstance().getModContainer(modId).orElse(null);if(container!=null){var path=container.getMetadata().getIconPath(size).orElse(null);if(path!=null){String p=path.replace('\\','/');String prefix="assets/"+modId+"/";if(p.startsWith(prefix))p=p.substring(prefix.length());Identifier id=Identifier.of(modId,p);c.drawTexture(RenderPipelines.GUI_TEXTURED,id,x,y,0,0,size,size,size,size);return;}}}catch(Throwable ignored){}RenderUtil.fill(c,x,y,x+size,y+size,ACCENT);RenderUtil.fill(c,x+4,y+4,x+size-4,y+size-4,0xFFFFFFFF);}
