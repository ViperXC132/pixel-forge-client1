...
public class AccountsScreen extends Screen {
    ...
    @Override protected void init(){
        ...
        userField.setPlaceholder(Text.literal("Username / email")); userField.setMaxLength(64);
        passField = new TextFieldWidget(textRenderer,20,0,width-40,20,Text.literal("Password"));
        passField.setPlaceholder(Text.literal("Password (empty for Offline)")); passField.setMaxLength(128);
        addDrawableChild(userField); addDrawableChild(passField);
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){
        ...
        for(Account a:AccountManager.getAccounts()){
            RenderUtil.fill(c,16,y,width-16,y+32,PANEL); RenderUtil.drawBorder(c,16,y,width-32,32,0xFF1E2540);
            ...
            if(!a.active) drawAction(c,width-76,y+8,58,16,"Switch",ACCENT); else RenderUtil.fill(c,width-30,y+13,width-24,y+19,0xFF40C057);
            y+=36;
        }
        y+=8; RenderUtil.drawText(c,textRenderer,"ADD / LOGIN",16,y,ACCENT,false); y+=15;
        ...
        int fieldY=y+21; userField.setY(fieldY); passField.setY(fieldY+27);
        userField.render(c,mx,my,d); passField.render(c,mx,my,d);
        if(!userField.getText().isBlank()) SkinHelper.drawHead(c,userField.getText().trim(),width-48,fieldY+1,22);
        int by=fieldY+54; drawAction(c,20,by,width-40,20,busy?"Working...":"Login & Apply Session",busy?MUTED:ACCENT);
        if(!status.isEmpty()) RenderUtil.drawText(c,textRenderer,status.startsWith("OK:")?status.substring(3):status,20,by+27,status.startsWith("OK:")?0xFF40C057:0xFFFA5252,false);
        RenderUtil.drawText(c,textRenderer,"Microsoft uses the secure device-code sign-in flow.",12,height-14,MUTED,false);
        super.render(c,mx,my,d);
    }

    private void drawTypeBtn(DrawContext c,int x,int y,String label,boolean on){
        int w=textRenderer.getWidth(label)+14; RenderUtil.fill(c,x,y,x+w,y+16,on?0x403B5BDB:PANEL);
        RenderUtil.drawBorder(c,x,y,w,16,on?ACCENT:0xFF1E2540); RenderUtil.drawText(c,textRenderer,label,x+7,y+3,on?0xFF748FFF:MUTED,false);
    }
    private void drawAction(DrawContext c,int x,int y,int w,int h,String label,int color){
        RenderUtil.fill(c,x,y,x+w,y+h,0x203B5BDB); RenderUtil.drawBorder(c,x,y,w,h,color);
    ...
    @Override public boolean mouseClicked(net.minecraft.client.gui.Click click,boolean doubled){
        double mx=click.x(), my=click.y(); int y=58;
        for(Account a:AccountManager.getAccounts()){
            if(!a.active && mx>=width-82 && my>=y && my<=y+32){ AccountManager.switchTo(a); status="OK: Switched to "+a.username; return true; }
            y+=36;
        }
        y+=8; int typeY=y+15;
        if(my>=typeY&&my<=typeY+16){
            ...
            setFocused(userField); return true;
        }
        int fieldY=typeY+21; int by=fieldY+54;
        if(!busy&&my>=by&&my<=by+20&&mx>=20&&mx<=width-20){
            ...
            busy=true; status="Authenticating..."; AccountManager.loginAsync(selectedType,user,pass,msg->{busy=false;status=msg;if(msg.startsWith("OK:"))passField.setText("");}); return true;
        }
