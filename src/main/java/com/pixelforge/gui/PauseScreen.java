...
public class PauseScreen extends Screen {
    ...
    private static final int BG=0xE60A0E17,ACCENT=0xFF7182FF,TEXT=0xFFEAF0FF,DIM=0xFF8490AA;
    ...
    @Override public void render(DrawContext c,int mx,int my,float d){
        layout();RenderUtil.fill(c,0,0,width,height,0x66000000);RenderUtil.fill(c,left,top,left+w,top+h,BG);RenderUtil.drawBorder(c,left,top,w,h,0xFF293454);
        RenderUtil.drawText(c,textRenderer,"PIXELFORGE",left+24,top+22,TEXT,false);RenderUtil.drawText(c,textRenderer,"PAUSED",left+24,top+40,ACCENT,false);
        ...
        RenderUtil.drawText(c,textRenderer,"Disconnect",left+24,y+8,DIM,false);RenderUtil.drawText(c,textRenderer,"ESC  Resume",left+w-112,y+8,DIM,false);super.render(c,mx,my,d);
    }
    private void button(DrawContext c,int x,int y,int bw,int bh,String label,int mx,int my){boolean hover=mx>=x&&mx<=x+bw&&my>=y&&my<=y+bh;RenderUtil.fill(c,x,y,x+bw,y+bh,hover?0x453B5BDB:0x24151D30);RenderUtil.drawBorder(c,x,y,bw,bh,hover?ACCENT:0xFF293454);RenderUtil.drawCenteredText(c,textRenderer,label,x+bw/2,y+11,hover?TEXT:DIM,false);}
    @Override public boolean mouseClicked(net.minecraft.client.gui.Click click,boolean doubled){
        layout();if(click.button()!=0)return super.mouseClicked(click,doubled);int bw=238,bh=34,gap=10,x1=left+24,x2=left+w-24-bw,y=top+68;
        ...
        if(click.x()>=left+20&&click.x()<=left+180&&click.y()>=y&&click.y()<=y+30){client.disconnect(Text.literal("Disconnected from server"));return true;}return super.mouseClicked(click,doubled);
    }
    private boolean hit(net.minecraft.client.gui.Click c,int x,int y,int w,int h){return c.x()>=x&&c.x()<=x+w&&c.y()>=y&&c.y()<=y+h;}
    private void openResourcePacks(){try{Path dir=client.getResourcePackDir();client.setScreen(new PackScreen(client.getResourcePackManager(),manager->{client.options.refreshResourcePacks(manager);},dir,Text.literal("Resource Packs")));}catch(Throwable t){client.setScreen(new Screen(Text.literal("Resource Packs unavailable")){});}}
